...
## INTRODUCTION
 ...
 - BMIs are good indicators of stream health
 - The statewide dataset provides opportunity to connect biology with hydrology (FF metrics)
 - By utilizing the statewide CSCI biologic index in at paired flow sites, we can assess patterns of alteration in functional flow metrics, and evaluate and identify which metrics best describe a gradient of alteration from an ecological flow perspective.
 - These metrics can be used to develop flow-ecology response relationships which can help inform flow management recommendations by helping to determine which parts of the hydrograph (components and metrics) management efforts should focus on
 - This approach provides a unique perspective on how alteration has impacted stream health/communities across the state

## METHODS

This provides an overview of the steps required to assess functional flow metrics in relation to BMI and ASCI ecological data. The basic overview requires spatially linked ecological stream data and associated functional flow metrics (FFM) for a given stream or stream segment. We used a functional flows approach, as defined by Yarnell et al. 2015 and Yarnell et al. 2020, to make relationships between hydrologic conditions and BMIs. The Functional Flows Calculator (eflows.ucdavis.edu/hydrology) provided functional flow metrics for flow data associated with each USGS gage used in this analysis. We generated a score of alteration based on the functional flow metrics.

### Steps to Pair BMI/ASCI Sites with Flow Sites

To make relationships between benthic macroinvertebrates (BMIs) and flow, identified all BMI monitoring sampling sites across the state, with sampling between 1994-2018 (SWAMP/SWCCRP). We then linked these locations with any USGS gage sites that both occurred in the same HUC12 as the BMI station, and existed on the same NHD mainstem streamline within 10 km. We then filtered to USGS gages that had at least ten years of daily flow data and had flow data post 1994 to generate functional flow metrics (Figure \@ref(fig:spatialWorkflow)).
...
Once data sites were paired, hydrologic data was used to calculate functional flow metrics, and an estimate of alteration status. Alteration status and functional flow metric data were joined with BMI StationCodes to spatially link the paired BMI-USGS sites, yielding hydrologically-linked BMI metric data (i.e., CSCI). To better understand temporal trends in the data, we split datasets into four distinct groups. 
...
```{r map2, echo=FALSE, eval=TRUE, out.width='30%', fig.cap="Map of all available USGS sites in California with more than 10 years of data"}
...
Based on modeling, the top variables correlated with alteration were identified using boosted regression trees (Table \@ref(tab:brtTab) ). 
...
Models converged on similar variable sets (Figure \@ref(fig:RIwide)).
...
Marginal effects plots for the top eight variables (Figure \@ref(fig:RImargfx)). 
