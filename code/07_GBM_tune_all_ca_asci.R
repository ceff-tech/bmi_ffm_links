...
bio_ffm %>%
  filter(bioindicator == "ASCI") %>% 
  # some extreme values so filter to everything below 98 quantile
  filter(delta_p50 > quantile(delta_p50, na.rm=TRUE, 0.95)) %>% 
  filter(delta_p50 > 30) %>% 
...
# tally sites by stream class
bio_ffm %>% filter(bioindicator==bioindi) %>%
  distinct(gageid, SampleID, .keep_all=TRUE) %>%
  select(class3_name) %>% table()
# MIXED   RAIN SNOWMELT 
# 88      231  37 

dim(data_por) # CSCI: 356 / 27
...
dim(data_por) # 3 sites dropped

summary(data_por) # still a sprinkling of NAs
...
# remove rows that have more than 70% NA
data_strmclass <- data_strmclass[which(rowMeans(!is.na(data_strmclass))>0.7),]
dim(data_strmclass) # drop 3

summary(data_strmclass)
...
bio_ffm %>% 
  ...
  scale_fill_viridis_d("FFM") + 
  coord_flip() +
  labs(subtitle = "ASCI: Snowmelt stream class sites (outliers >95 percentile excluded)") +
  theme(plot.background = element_rect(fill="white"))
