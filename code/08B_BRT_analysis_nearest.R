...
load("data_output/bmi_nearest_usgs_stations.rda") # bmi nearest usgs gage
load("data_output/selected_bmi_stations_w_comids.rda") # all distinct bmi stations
load("data_output/selected_usgs_gages.rda") # all usgs gages with bmi pairs
...
sel_gages_bmi <- st_join(st_transform(sel_gages_bmi, 3310), left = TRUE, regions["huc_region"])
bmi_nearest <- st_join(st_transform(bmi_nearest, 3310), left = TRUE, regions["huc_region"])

mapview(sel_gages_bmi, col.regions="deepskyblue4", cex=7, alpha=0.7) + 
  mapview(mainstems, color="darkblue", lwd=0.5) +
  #mapview(regions, zcol="huc_region", alpha.regions=0.3) + 
...
data_ann <- dplyr::select(bmi_flow_metrics_ann_csci, 1, 106, 151, 91:93, 96, one_of(bmi.metrics), 117:150) %>% 
  filter(StationCode %in% region_sel$StationCode) %>% 
  as.data.frame()

data_lag1 <- dplyr::select(bmi_flow_metrics_lag1_csci, 1, 106, 151, 91:93, 96, one_of(bmi.metrics), 117:150) %>% 
  filter(StationCode %in% region_sel$StationCode) %>% 
  as.data.frame()

data_lag2 <- dplyr::select(bmi_flow_metrics_lag2_csci, 1, 106, 151, 91:93, 96, one_of(bmi.metrics), 117:150) %>% 
  filter(StationCode %in% region_sel$StationCode) %>% 
  as.data.frame()

data_por <- dplyr::select(bmi_flow_metrics_por_csci, 1, 106, 151, 91:93, 96, one_of(bmi.metrics), 117:150) %>% 
  filter(StationCode %in% region_sel$StationCode) %>% 
  as.data.frame()
...
dat_shann <- data_ann[,c(8,18:ncol(data_ann))] #%>% 
  #select(-DS_No_Flow)
...
gbm1 <- My.gbm.step(data=dat_shann,
                 ...
                 n.folds = 10,           # 10 default for function; 5 was used in De'ath 2007, good for low n
...
# select dat
dat_shann1 <- data_lag1[,c(8,18:ncol(data_lag1))]
...
# select dat
dat_shann2 <- data_lag2[,c(8,18:ncol(data_lag2))]
...
# select dat
dat_shann_por <- data_por[,c(8,18:ncol(data_por))]
...
# select cols of interest
dat_csci <- data_ann[,c(15,18:ncol(data_ann))] %>% 
  filter(!is.na(csci_percentile)) %>% 
  select(-DS_No_Flow) %>% as.data.frame()
...
gbm1 <- My.gbm.step(data=dat_csci,
                    ...
                    family = "gaussian",   # the 'loss function', Gaussian minimizes mean square error
                    tree.complexity = 3,   # thus only models 3nd-order interactions
                    learning.rate = 0.0001, # the lower the learning rate/shrinking, more likely to over-fit
                    bag.fraction = 0.75,   # recommended in Elith and in Brown as top end of reasonable window
                    n.folds = 10,           # 10 default for function; 5 was used in De'ath 2007, good for low n
                    n.minobsinnode = 3)    # I edited gbm.step code to allow modification of n.minobsinnode
...
# select dat
dat_csci1 <- data_lag1[,c(15,18:ncol(data_lag1))] %>% 
  filter(!is.na(csci_percentile)) %>% 
  select(-DS_No_Flow) %>% as.data.frame()
...
# select dat
dat_csci2 <- data_lag2[,c(15,18:ncol(data_lag2))] %>% 
  filter(!is.na(csci_percentile)) %>% 
  select(-DS_No_Flow) %>% as.data.frame()
...
# select dat
dat_csci_por <- data_por[,c(15,18:ncol(data_por))] %>% 
  filter(!is.na(csci_percentile)) %>% 
  select(-DS_No_Flow) %>% as.data.frame()
