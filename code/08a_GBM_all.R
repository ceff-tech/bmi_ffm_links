...
load("data_output/06a_selected_bmi_flow_metrics_w_csci_ANN.rda")
load("data_output/06a_selected_bmi_flow_metrics_w_csci_POR.rda")
load("data_output/06a_selected_bmi_flow_metrics_w_csci_LAG1.rda")
load("data_output/06a_selected_bmi_flow_metrics_w_csci_LAG2.rda")
#load("data_output/05a_selected_bmi_stations_w_comids.rda")
...
bmi.metrics<-c("csci", "Taxonomic_Richness", "Shannon_Diversity", "EPT_Percent", "Clinger_Percent", "Coleoptera_Percent", "Predator_Percent",  "Intolerant_Percent")
...
data_ann <- dplyr::select(bmi_flow_metrics_ann_csci, 
                          1, 107, 91:94, 96:99,
                          # bug metrics
                          one_of(bmi.metrics), 
                          # flow metrics
...
data_lag1 <- dplyr::select(bmi_flow_metrics_lag1_csci, 
                           1, 107, 91:94, 96:99, 
                           one_of(bmi.metrics), 135:165) %>% 
  #filter(StationCode %in% region_sel$StationCode) %>% 
  as.data.frame()

data_lag2 <- dplyr::select(bmi_flow_metrics_lag2_csci, 
                           1, 107, 91:94, 96:99, 
                           one_of(bmi.metrics), 135:165) %>% 
  #filter(StationCode %in% region_sel$StationCode) %>% 
  as.data.frame()

data_por <- dplyr::select(bmi_flow_metrics_por_csci, 
                          1, 107, 91:94, 96:99, 
                          one_of(bmi.metrics), 135:165) %>% 
  #filter(StationCode %in% region_sel$StationCode) %>% 
  as.data.frame()
...
bmi_resp <- grep("Shannon_Diversity", names(data_ann))
...
dat_shann <- data_ann[,c(bmi_resp, 19:ncol(data_ann))]
...
# select dat
dat_shann1 <- data_lag1[,c(bmi_resp, 19:ncol(data_lag1))]
...
# select dat
dat_shann2 <- data_lag2[,c(bmi_resp, 19:ncol(data_lag2))]
...
# select dat
dat_shann_por <- data_por[,c(bmi_resp, 19:ncol(data_por))]
