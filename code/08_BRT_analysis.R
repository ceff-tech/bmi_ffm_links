...
ggplot() +
  geom_violin(data=data_ann, aes(x=stream_class, y=Shannon_Diversity, fill=csci), draw_quantiles =c(0.25, 0.5, 0.75) ) +
  geom_jitter(data=data_ann, aes(x=stream_class, y=Shannon_Diversity, fill=csci), pch=21, alpha=0.7) +
  theme_bw() + 
...
library(cowplot)
(shann_ri_combined <- plot_grid(shann_ri_ann, shann_ri_lag1, shann_ri_lag2, nrow = 2))
...
shann_RI_all <- bind_rows(gbm1_RI, gbm2_RI, gbm3_RI)
...
save(shann_RI_all, shann_gbm1, shann_gbm2, shann_gbm3, file = "data_output/gbm_shannon_RI_all.rda")
...
library(cowplot)
(taxon_ri_combined <- plot_grid(taxon_ri_ann, taxon_ri_lag1, taxon_ri_lag2, nrow = 2))
...
(shann_taxon_ri_combined <- plot_grid(
  taxon_ri_ann, taxon_ri_lag1, taxon_ri_lag2,
  shann_ri_ann, shann_ri_lag1, shann_ri_lag2,
  nrow=2))

save_plot(shann_taxon_ri_combined, filename="figs/brt_shann_taxon_top_RI_barplot_combined.png", base_height = 8)

# bind all the RI and plot later:
taxon_RI_all <- bind_rows(gbm1_RI, gbm2_RI, gbm3_RI)
...
# save
save(taxon_RI_all, taxon_gbm1, taxo_gbm2, taxon_gbm3, file = "data_output/gbm_taxonrichness_RI_all.rda")
...
(csci_ri_combined <- plot_grid(csci_ri_ann, csci_ri_lag1, csci_ri_lag2, nrow = 2))
...
# save
save(csci_RI_all, csci_gbm1, csci_gbm2, csci_gbm3, file = "data_output/gbm_csci_RI_all.rda")
...
library(cowplot)
(ept_ri_combined <- plot_grid(ept_ri_ann, ept_ri_lag1, ept_ri_lag2, nrow = 2))
...
(shann_taxon_csci_ept_ri_combined <- plot_grid(
  ...
  shann_ri_ann, shann_ri_lag1, shann_ri_lag2,
  csci_ri_ann, csci_ri_lag1, csci_ri_lag2,
  ept_ri_ann, ept_ri_lag1, ept_ri_lag2,
  nrow=4))
...
# bind all the RI and plot later:
ept_RI_all <- bind_rows(gbm1_RI, gbm2_RI, gbm3_RI)
#View(ept_RI_all)
...
# save
save(ept_RI_all, ept_gbm1, ept_gbm2, ept_gbm3, file = "data_output/gbm_ept_RI_all.rda")
...
library(cowplot)
(tol_ri_combined <- plot_grid(tol_ri_ann, tol_ri_lag1, tol_ri_lag2, nrow = 2))
...
(shann_taxon_csci_tol_ri_combined <- plot_grid(
  ...
  shann_ri_ann, shann_ri_lag1, shann_ri_lag2,
  csci_ri_ann, csci_ri_lag1, csci_ri_lag2,
  tol_ri_ann, tol_ri_lag1, tol_ri_lag2,
  nrow=4))
...
# bind all the RI and plot later:
tol_RI_all <- bind_rows(gbm1_RI, gbm2_RI, gbm3_RI)
#View(tol_RI_all)
...
# save
save(tol_RI_all, tol_gbm1, tol_gbm2, tol_gbm3, file = "data_output/gbm_tol_RI_all.rda")
...
save(bmi_RI_combined, file = "data_output/gbm_bmi_metrics_RI_combined.rda")
