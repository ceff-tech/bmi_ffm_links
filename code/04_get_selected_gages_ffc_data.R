...
bmi_coms_final <- bmi_coms_final %>% st_drop_geometry()

# # how many distinct gages? (n=512)
bmi_coms_final %>% distinct(ID) %>% tally()
 
# # get a usgs_list
usgs_list_paired <- bmi_coms_final %>% dplyr::distinct(ID, .keep_all=TRUE) %>%
  select(ID, StationCode, comid, comid2, lon, lat) %>%
