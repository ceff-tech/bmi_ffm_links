...
sum(unique(bmi_csci_por_trim$site_id) %in% as.character(gages_ref_to_alt$gage))
usgs_ref_to_alt <- bmi_csci_por_trim %>% 
  filter(site_id %in% as.character(gages_ref_to_alt$gage)) %>% 
  distinct(site_id)
write_csv(usgs_ref_to_alt, file="data_output/04_usgs_ref_to_alt_gages_followup.csv")
...
# plot CSCI w/ NAs
(g1 <- ggplot(data=bmi_csci_por_trim %>% 
  ...
  labs(y="CSCI", x="CEFF Gage Type", subtitle="CSCI Score by FFC Alteration Status & Gage Type")+
...
# plot CSCI w/ NAs
(g2 <- ggplot(data=bmi_csci_por_trim %>% 
         filter(status!="not_enough_data"), 
       aes(x=status, y=csci)) + 
  geom_boxplot(aes(fill=status), show.legend = F, notch = TRUE) +
  stat_summary(fun.data=stat_box_data, geom="text", cex=3, hjust=1, vjust=0.9) +
  labs(y="CSCI", x="Alteration Status", subtitle="CSCI Score by FFC Alteration Status")+
  theme_bw(base_family = "Roboto Condensed") + 
  #facet_grid(.~status) +
  scale_fill_colorblind())

ggsave(filename = "figs/04_csci_scores_by_ffc_alteration_status.png", height = 8, width = 11, units = "in", dpi=300)
