...
# check class
class(bmi_clean_stations)
class(bmi_clean_stations_ss)
...
# Add H12 to points to BMI and Gages (adds ATTRIBUTES, retains ALL pts if left=TRUE), using BMI_CLEAN DISTINCT W SS
#bmi_h12 <- st_join(bmi_clean_stations_ss, left = TRUE, h12[c("HUC_12","h12_area_sqkm")]) #
...
save(sel_h12_bmi, file="data_output/03_selected_h12_contain_bmi_gage.rda")
