...
hyper_grid <- expand.grid(
  shrinkage = c(.005, .009), # c(0.001, .005, .009)  
  interaction.depth = c(2,4), #c(1,2,3),
...
capture.output(gbm_fin_out <- purrr::pmap(
  ...
  ), file=paste0("data_output/gbms/10_gbm_final_",as_name(bmiVar),"_", tolower(hydroDat),"_output.txt"), append=T)
...
cat("\nBest parameters for GBM.STEP:\n\n", 
    file = paste0("data_output/gbms/10_gbm_final_",as_name(bmiVar),"_", tolower(hydroDat),"_output.txt"), append=TRUE)
...
# get file name
(fileToSave <- ls(pattern = paste0("gbm_final_", as_name(bmiVar))))

# save to RDS
write_rds(ls(pattern = fileToSave), path = paste0("data_output/gbms/10_",fileToSave, ".rds"), compress = "gz")
