...
bioindi <- "ASCI"
bmi.metrics<-c("asci") # response var for model
hydroDat <- "POR" # dataset, can be Annual, Lag1, Lag2, POR
bmiVar <- quote(asci) # select response var from list above
...
data_por_train <- data_por %>% # use all data
  dplyr::select(biovalue, 10:ncol(.)) %>%  
  # use 12 if not including HUC region and CEFF_type
  dplyr::filter(!is.na(biovalue)) %>% as.data.frame()
         
# double check cols are what we want
names(data_por_train) # should be 20 (ASCI + MP_metric, class3_name, Power.avg and 16 metrics)
str(data_por_train)
